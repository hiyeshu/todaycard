...
styles.css - 视觉层，提供淡彩灰格移动界面、圆角质感卡片 coverflow、默认正面 10x10 网格、答案页同源实色符号、两行答案位、大圆框输入、右侧状态文字游戏抽卡按钮、灰色源仓库链接、隔离层逐张发牌动效和翻牌状态
...
app.js - 行为层，根据单句决策自动生成候选选项和带品牌、选项编号、月日的 seeded cards，消费 data.js 的 palette、选项编号与图案，生成默认正面网格和答案页同源实色符号，驱动拖拽牌堆、逐张发牌、原地翻牌和音效事件
...
skills/todaycard/assets/todaycard-single.html - 单 HTML 模板资产，内联 TodayCard favicon、结构、默认正面 10x10 网格、答案页同源实色符号、行为、游戏化抽卡按钮、发牌动效和 Web Audio 仪式音效

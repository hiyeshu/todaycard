...
package.json - 构建边界，`npm run build` 只复制公开站点文件、data.js、audio.js、favicon、OG PNG、robots、sitemap 和 manifest 到 dist，避免 skill 和架构文档被 Pages 发布
.gitignore - 版本控制边界，忽略 dist 构建产物和 Wrangler 本地缓存，保持源码仓库干净
README.md - 中文主入口文档，顶部提供 English 切换链接，先展示自有 favicon 与 TodayCard 标题，再展示产品 OG 图，并说明运行、Dify、部署、skill 安装和完整性检查边界
README.en.md - 英文 GitHub 入口文档，顶部提供中文切换链接，镜像 README 的运行、Dify、部署、skill 和开发边界
...
assets/og.png - 1200x630 imagegen 产品化社交预览图，展示真实 TodayCard 牌堆、输入框、抽卡按钮和产品数据
...
skills/todaycard/SKILL.md - Agent skill 入口，定义 TodayCard 类任务的触发条件、决策追问、4 个答案生成、网页端 App 提示、单 HTML 注入预览和安装查漏边界
...
skills/todaycard/assets/todaycard-single.html - 单 HTML 模板资产，内联 TodayCard favicon、结构、可选 `window.TodayCardPreload` 决策/答案预注入、移动端一屏锁定且与输入同轴的循环 full-bleed coverflow、默认正面 10x10 网格、答案页同源实色符号、选卡/翻牌行为、游戏化抽卡按钮、发牌动效和 Web Audio 仪式音效
skills/todaycard/references/CLAUDE.md - L2 skill references 地图，记录规则文档职责边界
skills/todaycard/references/install-check.md - 安装完整性查漏协议，检查 `npx skills` 安装后模板、资产、契约和 UI 元数据是否齐全
skills/todaycard/references/todaycard-contract.md - TodayCard 细节契约，记录 skill 运行、4 个答案生成、网页端 App 提示、数据、视觉、交互、单 HTML 和安装完整性
...
TodayCard 的真相链是：内容先成为稳定数据，再进入唯一网页。`index.html` 只做极简入口，`styles.css` 保留白底 coverflow 语言并改成圆角实体卡，`assets/patterns.md` 定义 10x10 图案语义，`data.js` 管 palette、选项编号和图案数据，`app.js` 管 Dify/本地候选、seeded renderer 和发牌状态，`audio.js` 管抽卡、发牌、滑卡和翻牌反馈音效，`functions/api/cards.js` 只做 Dify 密钥代理，`skills/todaycard/` 把这套产品契约包装成可被 `npx skills` 发现的标准能力包，`skills/todaycard/references/install-check.md` 记录安装查漏边界，`skills/todaycard/assets/todaycard-single.html` 提供可复制的单文件交付资产，`https://todaycard.app/` 则作为无需本地安装的线上运行入口。

## 开发规范

- 选项图案必须由 `decision + option + index` 生成稳定 seed。
- 默认只要求用户输入一个决策题；不暴露自定义选项输入，候选项由 Dify 或本地逻辑生成。
- SEO 真相源在 `index.html` 的 head、`robots.txt`、`sitemap.xml`、`site.webmanifest` 和 `assets/og.png`；canonical 必须指向 `https://todaycard.app/`。
- 首次打开页面只渲染本地默认卡；只有用户点击“抽卡”或按 Enter 才允许请求 Dify。
- 决策输入默认空值，只显示按浏览器语言切换的 placeholder；中文为 `今天想决定什么？`，英文为 `What do you want to decide today?`，输入组件只保留大圆框，不预填具体问题。
...
- Cloudflare Pages 公开输出必须来自 `dist/`；只发布 `index.html`、`styles.css`、`data.js`、`audio.js`、`app.js`、`robots.txt`、`sitemap.xml`、`site.webmanifest`、`assets/todaycard.svg` 和 `assets/og.png`，`skills/`、`CLAUDE.md`、`AGENTS.md` 和 references 只进仓库，不进站点。
...
- 安装 skill 后怀疑缺文件时，先执行 `skills/todaycard/references/install-check.md`，不能把 `npx skills` 发现成功当成模板、资产和契约齐全。
- 用户只想体验或分享产品时，运行入口优先给 `https://todaycard.app/`；但线上可访问不能替代本地 skill 完整性检查。
...
- 2026-06-23: 将 skill 收敛到 `skills/todaycard/`，补全单 HTML 模板资产，并为 skill assets 创建 L2 地图。
...
- 2026-06-24: 新增 README 项目入口，使用自有 favicon 和产品 OG 图，并记录本地运行、Dify、部署和 skill 边界。
- 2026-06-24: 将公开 OG 真相收敛为 `assets/og.png`，使用 imagegen 生成贴近真实产品设计的预览图，并保留 `assets/todaycard.svg` 作为 favicon。
- 2026-06-24: 调整 README 顶部顺序为 favicon 标题优先、产品图第二，并删除彩色 badge 行。
- 2026-06-24: 新增 skill 安装完整性查漏协议，并把 `https://todaycard.app/` 写入运行入口分支。
- 2026-06-24: 将 `todaycard` skill 默认运行路径收敛为接收或追问一个决策题、生成 `Choice A-D` 四个答案，并提示网页端 App 手机可访问。
- 2026-06-24: 将 skill 单 HTML 路径改为复制 `todaycard-single.html`、注入 `window.TodayCardPreload` 决策/答案并打开预览，避免引用 skill 包内不存在的 split JS。
- 2026-06-25: 主站 i18n 改为只按浏览器语言判定，中文浏览器显示中文 placeholder/抽卡/发牌中，其他浏览器显示英文，不改动 skill 单 HTML 模板。
- 2026-06-25: 新增英文 `README.en.md`，中文 `README.md` 保持主入口并在顶部提供 English 切换，英文文档回链中文入口。
